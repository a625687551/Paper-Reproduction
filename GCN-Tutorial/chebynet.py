...
graph_lambda_max = LaplacianMaxEigenvalue(graph.x, graph.edge_index,
                                          graph.edge_weight)

model = tfg.layers.ChebyNet(64, K=3, lambda_max=graph_lambda_max(
    normalization_type="rw"))
...
def compute_loss(logits, mask_index, vars):
    ...
    l2_losses = [tf.nn.l2_loss(kernel_val) for kernel_val in kernel_vals]
